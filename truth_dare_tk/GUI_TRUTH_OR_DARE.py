...
def sub_p_n_2():
    ...
    #        label_or = Label(0,text="Or")
            ...
            label_choice.grid(row=4,column=1)
    #        label_or.grid(row=3,column=3)
            truth_button.grid(row=4,column=2)
            dare_button.grid(row=4,column=4)

        win.destroy() #destroy the first window

        sec_win = Tk() #getting to a second window
        sec_win.minsize(400,400)
        sec_win.title("Truth Or Dare")
#        sec_win.resizeble(False,False)
    ...
    if p_n_1.get() == p_n_2.get() or p_n_1.get() == p_n_3.get():
        ...
        def get_player():
            ...
            def sh_truths():
                ...
                label_truth = Label(0,text="The truth you have say>> "+random.choice(t), font=('times', 12, 'bold'))
                label_truth.grid(row=2)
            ...
            def sh_dares():
                ...
                label_dare = Label(0,text="The dare you have do>> "+random.choice(d), font=('times', 12, 'bold'))
                label_dare.grid(row=2)

                btn_pro = Button(sec_win, text="Proceed", command=proceeding)
                btn_pro.grid(row=3)

            label_p_n = Label(0,text="It's "+random.choice(players).capitalize()+"'s turn", font=('times', 8, 'bold'))
